...
// Server represents the routes and functionality of the AegisCampus API.
...
func (as *Server) registerRoutes() {
	...
	router.HandleFunc("/campaigns/{id:[0-9]+}/results/{rid}/report", as.CampaignResultReport).Methods("POST")
	...
	router.HandleFunc("/redirect_pages/", as.RedirectPages)
	router.HandleFunc("/redirect_pages/{id:[0-9]+}", as.RedirectPage)

	// Video Management API
	router.HandleFunc("/videos/", as.HandleVideos).Methods("GET", "POST")
	router.HandleFunc("/videos/{id:[0-9]+}", as.HandleVideoByID).Methods("GET", "PUT", "DELETE")
	...
	router.HandleFunc("/users/me/preferences", as.UserPreferencesMe).Methods("GET", "PUT")
