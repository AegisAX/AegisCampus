...
AegisCampus implements simple Role-Based-Access-Control (RBAC) to control access to
certain resources.

By default, AegisCampus has two separate roles, with each user being assigned to
a single role:
...
const (
	// RoleAdmin is used for AegisCampus system administrators. Users with this
	// role have the ability to manage all objects within AegisCampus, as well as
	// system-level configuration, such as users and URLs.
	RoleAdmin = "admin"
	// RoleUser is used for standard AegisCampus users. Users with this role can
	// create, manage, and view AegisCampus objects and campaigns.
	RoleUser = "user"

	// PermissionViewObjects determines if a role can view standard AegisCampus
...
// Role represents a user role within AegisCampus. Each user has a single role
